...
const CreatePlantModal = ({ isModalOpen, onClose, setRefetch, isRefetch }) => {
  const {
    control,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit = async (data: PlantFormInputs) => {
    try {
      ...
      const user_id = Cookies.get("user_id") || 1; // Default/fallback user ID

      if (!token) {
        Swal.fire({ text: "No token found.", icon: "error" });
        return;
      }
      ...
      Swal.fire({
        text:
          message === "Plant code already exists"
            ? "Plant code must be unique. That code is already used."
            : message || "Failed to add plant. Please check your data.",
        icon: "error",
      });
    }
  };
  ...
  return (
    <Modal isModalOpen={isModalOpen}>
      <div className="modal-header">
        <h3 className="modal-title">Add Plant</h3>
            ...
            {inputFields.map(({ name, label, placeholder, icon: Icon }) => (
              <div className="form-group mb-2" key={name}>
                ...
                <div className="flex items-center border rounded-lg shadow-sm bg-white">
                  ...
                  <Controller
                    name={name as keyof PlantFormInputs}
                    control={control}
                    render={({ field }) => (
                      <input
                        {...field}
                ...
                {errors[name as keyof PlantFormInputs] && (
                  <p className="text-red-500 text-sm mt-1">
                    {errors[name as keyof PlantFormInputs]?.message}
                  </p>
                )}
              </div>
            ))}
