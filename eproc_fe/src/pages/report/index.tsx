...
export default function ReportRFQPage() {
  const [statusFilter, setStatusFilter] = useState("all");
  const [tableData, setTableData] = useState<IRFQReport[]>([]);
  const [isExporting, setIsExporting] = useState(false);

  // Function to transform and filter data
  const transformData = (data: IRFQReport[], search: string) => {
    const now = moment();
    ...
    const filtered = data
      ...
      .filter((item) =>
        ...
        (
          ...
          item.vendor_email?.toLowerCase().includes(searchLower)
        )
      );
  ...
  const additionalTransform = useCallback((data: IRFQReport[], search: string) => {
  ...
  const columns: ColumnDef<IRFQReport>[] = [
    ...
    { accessorKey: "vendor_email", header: "Vendor Email" },
    {
      accessorKey: "price",
      header: "Price",
      cell: (info) =>
  ...
  const downloadExcel = async () => {
    try {
      ...
      const filename = `RFQ_Report_${currentDate}.xlsx`;
    ...
    <Main>
      <div className="p-6">
        <div className="flex justify-between items-center mb-4">
          <h1 className="text-2xl font-semibold text-gray-800">Report RFQ</h1>
        ...
        <DataTable
          columns={columns}
          url={`${process.env.NEXT_PUBLIC_API_URL}/api/report`}
          isRefetch={false}
